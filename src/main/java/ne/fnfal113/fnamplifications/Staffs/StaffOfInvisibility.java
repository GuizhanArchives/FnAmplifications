...
public class StaffOfInvisibility extends LimitedUseItem {
    ...
    private static final NamespacedKey usageKey = new NamespacedKey(FNAmplifications.getInstance(), "invistaff");

    public StaffOfInvisibility(ItemGroup itemGroup, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe) {
        super(itemGroup, item, recipeType, recipe);

        setMaxUseCount(value.staffOfInvisibility());
    }
    ...
    public @Nonnull ItemUseHandler getItemHandler() {
        return e -> {
            ...
            Util.send(p, "&你获得了超级隐身，所有玩家都无法看到你!");

            Objects.requireNonNull(p.getLocation().getWorld()).playSound(p.getLocation(), Sound.ENTITY_ILLUSIONER_CAST_SPELL, 1, 1);
